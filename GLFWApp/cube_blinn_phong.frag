...
//**** Implementation of the Blinn-Phong shading model 
...
float specular_exponent = 50.0; // the specular shiness, higher value with smaller highlights
...
void main()
{
	...
	// Commpute the half-way direction in the eye space
	vec3 half_way_dir = normalize(light_direction + eye_dir);
	
	// Compute the shiness term
	float specular_dot_prod = max(dot(half_way_dir, cube_normal_eye_space), 0.0);
